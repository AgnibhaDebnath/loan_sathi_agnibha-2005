...
const LoanApplications = () => {
    ...
    const updateStatus = async (permission) => {
        if (permission) {
            try {
                const res = await fetch("https://loan-sathi.onrender.com/update-status", {
                    method: "POST",
                                        ...
                                        loanApplications.length === 0 ? (
                                            ...
                                            <tr key={loan.id} className="border text-gray-800">
                                                <td className="border-r  border-gray-300 p-2 text-left text-base font-medium"><span className="block">
                                                    ...
                                                    <span className="block ml-2"><Phone className="inline-block text-green-500" size={18} />{loan.mobile_no.slice(0, -4).replace(/./g, "X") + loan.mobile_no.slice(-4)}</span>
                                                </td>
